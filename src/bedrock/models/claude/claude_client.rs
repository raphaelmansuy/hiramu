...
use crate::bedrock::models::claude::claude_request_message::{
    Content, ContentBlock, Message, RequestMessage,
};
...
impl ClaudeClient {
    ...
    /* *    pub async fn chat(
        ...
        let request = RequestMessage::new(options.max_tokens, messages.to_vec())
            ...
            .with_stop_sequences(options.stop_sequences);

        let response = self.bedrock_client.generate_raw(request).await?;

        // Parse the response and create a ChatResponse instance
...
fn parse_chat_response(response: Value) -> Result<ResponseMessage, ClaudeError> {
    ...
    let content_blocks: Result<Vec<ContentBlock>, ClaudeError> = content
        .iter()
        .map(|block| {
